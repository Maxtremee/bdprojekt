...
def getStudents():
    connection = cx_Oracle.connect("pwr_19_20_L_018248874", "Colvar6808", "156.17.43.90", encoding="UTF-8")
...
def getTeachers():
    ...
    teacher_index = int(input('Wybierz indeks ucznia : '))
...
def getStudentSchedule(id_ucznia):
    ...
    for item in sorted(data, key = lambda i: (int(i['beg'][0]), int(i['beg'][1]))):
        print(str(i)+'  > '+str(item['subject'])+' > '+str(item['beg'][0]+':'+item['beg'][1])+'   '+str(item['end'])+'   '+str(item['name']))
        i+=1
...
def mailBox(id_receiver):
    ...
    if choice == '2':
        ...
        if choice == '2':
            ...
            cursor.execute(sql, {'sender_id':id_receiver, 'receiver_id':data[message_index]['id'], 'message':message})
            connection.commit()
...
def printTeacherStudents(teacher_id):
    ...
    if choice == '1':
        ...
        if choice == '2':
            ...
            sql = "UPDATE oceny SET ocena = "+str(new_grade)+"SET opis = "+new_description+" WHERE id_oceny = "+str(grade_id)
            cursor.execute(sql)
            connection.commit()
